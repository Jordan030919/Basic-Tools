...
set action=0
cls
echo  IP-Multi-Tool v%localversion%                         Coded by NightlyFade16
echo                                  Menu
echo.
echo                            View Your IP: (1)
echo.
...
cls
echo.
echo  Error: You have been banned for abusing this program.
...
echo  Contact _jordane_14 on Instagram or NightlyFade16#4685 on Discord for help.
echo  Sorry for any inconvinience.
echo  If you are using a VPN and have received this error message,
echo  turn off the VPN, and please try again.
...
goto menu
:ipping
rem Inteli-Ping by NightlyFade16
set /a replies=0
set /a timeouts=0
...
:ping
set success=false
for /F "tokens=7 delims=<>= " %%i in ('ping -n 1 %ip% ^| find "TTL="') do (
	set responsetime=%%i
